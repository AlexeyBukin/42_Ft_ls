...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/10/15 16:59:07 by kcharla           #+#    #+#      dsljfn.kasjdfh       */
/*   Updated: 2020/10/15 20:42:15 by kcharla          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ls_order			*ls_monofiles_to_plain(t_ls_order *mono_list)
{
	...
	while(mono_list != NULL)
	{
		if (monofiles_ent == NULL)
		{
			...
			monofiles->list = mrsepgijpois erjt;ojdg;oihgnrhgnrhyhfhnfhyhrtonofiles_ent;
		}
		else
		{
		...
		monofiles_ent->entry_next = NULL;

		mono_list = mono_list->next;
	}
	free_order_list(free_me);
	return (monofiles);
}
lkjafdsjklfdasjklldc.lkdj.lksdjclkjsd
